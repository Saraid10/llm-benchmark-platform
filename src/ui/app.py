"""
HuggingFace Spaces entrypoint - minimal, dependency-safe version.
"""
import sys, os
...
def query(hw_ram, hw_has_gpu, hw_gpu_vram, sel_quants, sel_models, max_mem):
    ...
    if df.empty:
        empty = pd.DataFrame(columns=["model_id","quantization","tokens_per_sec","memory_used_mb"])
        fig   = px.bar(title="No data for this selection")
        return empty, fig, fig
    
    cols = ["model_id","quantization","hardware_profile","tokens_per_sec","memory_used_mb","latency_avg_ms"]
    fig1 = px.bar(df, x="model_id", y="tokens_per_sec", color="quantization",
                  barmode="group", title="Throughput (tokens/sec)")
    fig2 = px.bar(df, x="model_id", y="memory_used_mb", color="quantization",
                  barmode="group", title="Memory Usage (MB)")
    return df[cols], fig1, fig2

stats      = engine.get_db_stats()
all_models = engine.get_available_models() or ["mistral-7b-v0.1","llama-3.2-3b","phi-3-mini","gemma-2b"]
...
Compare quantized LLM performance across real hardware | **{stats['total_records']} records · {stats['models']} models**
""")
    with gr.Row():
    ...
    btn.click(fn=query, inputs=[hw_ram, hw_gpu, hw_vram, sel_quants, sel_models, max_mem],
